...
class schedule:
    ...
    def Schedule(self):
        ...
        t_duration = self.task_list.Peek(reverse=True).duration - current_progress
        ...
        while len(self.task_list) != 0:
            ...
            if p_duration >= t_duration: 
                schedule[current_date-1].append((self.task_list.Peek(reverse=True).name, current_time, current_time + t_duration))
                ##print(schedule)
                ...
                if self.CheckExpire(self.task_list.Peek(reverse=True).deadline_date, self.task_list.Peek(reverse=True).deadline_time, current_date, current_time):
                    ...
                    return self.ExpirationHandling(self.task_list.Peek(reverse=True))
                current_progress = 0
                current_non_consecutive = self.task_list.Peek(reverse=True).non_consecutive_type
                self.task_list.RemoveMin(reverse=True)
